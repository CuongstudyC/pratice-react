...
export default function SideRight() {
  ...
  const { cart, setCart, order } = useGlobal();
  ...
  // }
  useEffect(() => {
    setCartQuantity((product && cart[product.id]) ? cart[product.id] : 0);
      ...
      <div className="content-product">
        ...
        <div className="quantity">
          ...
          <div className="button-group">
            <button onClick={() => setCartQuantity(cartQuantity === 0 ? 0 : cartQuantity - 1)}>
              -</button>
              {/* contentEditable="true": có thể chỉnh sửa */}
              <button>{cartQuantity}</button>
            <button onClick={() => setCartQuantity(cartQuantity === 100 ? 100 : cartQuantity + 1)}>+</button>
          </div>

        </div>

        <div className="cart-checkout">
          <button onClick={() => setCart((product && cartQuantity > 0) ? { ...cart, [product.id]: cartQuantity } : { ...cart })}>
